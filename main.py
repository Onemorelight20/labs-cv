...
def main():
    ...
    for i in range(H):
        for j in range(W):
            ...
            # ---------------------------------------------------------------
            # Faces: Laplacian(Y) = 0 — standard 5-pt stencil.
            # Sobel Laplacian (5x5) is mathematically derived from Kx*Kx + Ky*Ky but
            # has dominance ratio 0.375/1.375 ≈ 0.27 => exponential blow-up.
            # Standard 5-pt is exactly diagonally dominant (center = off-diag sum),
            # which is the stability boundary required for sparse direct solvers.
            # ---------------------------------------------------------------
            elif is_face[i, j]:
                count = 0
                if i > 0:   count += 1; add_coef(k, k-W, 1.0)
                if i < H-1: count += 1; add_coef(k, k+W, 1.0)
                if j > 0:   count += 1; add_coef(k, k-1, 1.0)
                if j < W-1: count += 1; add_coef(k, k+1, 1.0)
                add_coef(k, k, -count)
                b[k] = 0.0

            # ---------------------------------------------------------------
            # Vertical Edges: dY/dy = 1/cos(theta)
            # In image coords, moving down (i+1) increases Y by 1/cos(theta).
            # Uses simple forward 2-pt diff: Y[i] - Y[i+1] = 1/cos(theta).
            # Sobel half-stencil [1,2,1]/4 has dominance ratio 0.5/1.5 = 0.33,
            # causing exponential blow-up over any connected edge chain.
            # ---------------------------------------------------------------
            elif is_vertical_edge[i, j]:
                add_coef(k, k, 1.0)
                if i < H-1: add_coef(k, k+W, -1.0)
                b[k] = 1.0 / cos_t

            # ---------------------------------------------------------------
            # Horizontal Edges: t · grad(Y) = 0,  t = tangent = (-ny, nx)
            # Upwind 2-pt finite difference along each tangent component.
            # Diagonal = 1e-4 + |t_x| + |t_y| ensures diagonal dominance:
            #   center >= off_diag (equality holds for unit tangents).
            # This is why the original formulation was numerically stable.
            # ---------------------------------------------------------------
                ...
                diag = 1e-4
                if t_x > 0: diag += t_x; add_coef(k, k-1, -t_x)
                else:        diag -= t_x; add_coef(k, k+1,  t_x)
                if t_y > 0: diag += t_y; add_coef(k, k-W, -t_y)
                else:        diag -= t_y; add_coef(k, k+W,  t_y)
                add_coef(k, k, diag)
                b[k] = 0.0
